    ...
    return (
        <div className="section-title section-title-first">
            <div className='add-link'>
                ...
                <input name='addLink' value={newLink} type='text' onChange={(event) => onAddInputChange(event)} placeholder='링크를 추가해 보세요'/>
                <button type='button' onClick={(event) => handleListClick(event, '폴더에 추가','폴더에 추가',newLink)}>추가하기</button>
            </div>
        </div>
    );
};
