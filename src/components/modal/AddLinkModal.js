...
const AddLinkModal = ({ isOpen, closeModal, link }) => {
  const folders = useGetFolders();
  const [selectedFolder, setSelectedFolder] = useState(null);

  const handleSelecteFolder = (folderId) => {
          ...
          {folders?.map((item) => {
            ...
            return (
              <FolderItems
                ...
                onClick={() => handleSelecteFolder(item.id)}
              >
                <FolderItem>
...
const FolderItems = styled.button`
  display: flex;
  width: 264px;
  padding: 8px;
  ...
  background: ${({ isSelected }) =>
    isSelected ? "var(--linkbrary-bg, #F0F6FF)" : "none"};
