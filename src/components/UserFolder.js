...
const UserFolder = ({folderDataObject, folderId=9999, handleListClick}) => {
    ...
    const folderOption = [
        ...
        {title: "이름변경", image: penImg, name: "폴더 이름 변경", btn:'변경하기'},
    ...
    return (
        <div className='user-folder'>
            <div className='folder-lists'>
                ...
                <h4 onClick={(event) => handleListClick(event, '폴더 추가','추가하기')}><span >폴더 추가</span><img src={addImg} alt="추가이미지"/></h4>
            </div>
            <div className='select-folder'>
                <h2>{titleName && titleName}</h2>
                {titleName && titleName !== "전체" ? <ul>
                {folderOption.map((option, index) => {
                    return <li key={index} onClick={(event) => handleListClick(event, option.name, option.btn, titleName)}> 
