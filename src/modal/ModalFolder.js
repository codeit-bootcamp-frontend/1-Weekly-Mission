    ...
    return (
        <div className='folder-modal'>
            <div className='folder-modal-wrap'>
                <img className='close-btn' src={closeImg} alt='닫기버튼아이콘' onClick={()=> handleClose()}/>
                <h2>{title}</h2>
                ...
                {title !== '폴더 공유' ? <button type='button' style={{background: isDelete() > -1 ? "var(--linkbrary-red)" 
                : "linear-gradient(91deg, #6D6AFE 0.12%, #6AE3FE 101.84%)"}}>{btnName}</button> : null}
