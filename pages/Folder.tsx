...
function Folder() {
  ...
  const handleClickUpdate = (value: string, key: string) => {
    setFolderName(value);
    setNowFolderId(key);
  };
  ...
  const searchingLink = userLinks.filter((link: FolderLinkData) => {
    if (search.length > 0) {
      if (
    ...
    <>
      ...
      <S.SelectPart>
        ...
        <LinkInfo
          ...
          nowFolderId={Number(nowFolderId)}
      ...
      {searchingLink.length !== 0 ? (
        <S.Main>
          <S.Section>
            {searchingLink.map((card: FolderLinkData) => {
