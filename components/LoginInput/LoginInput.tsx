...
export default function LoginInput({type}) {
  const {label, id, placeholder, message, Regex, confirm, src } = inputType(type);
  const {register, formState: {errors}, trigger, clearErrors} = useFormContext();
  ...
  const togglePasswordVisible = () => {
    ...
    setSrcToggle((prevSrcToggle) => (prevSrcToggle === src.eyeOff ? src.eyeOn : src.eyeOff));
  }
    ...
    <>
      <Style.Container>
        ...
        <Style.InputBox color={color}>
          ...
          /> 
          {src && <Image className="eyeImage" src={srcToggle} alt="보기" width={16} height={16} onClick={togglePasswordVisible} />}
        </Style.InputBox>
